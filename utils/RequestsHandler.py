...
class RequestsOperate:
    ...
    def _send_msg(self):
        """
        发请求
        :return: 返回该条用例的期望返回值except，所有响应内容json格式
        """
    ...
    def _write_cookies(self, response):
        ...
        for item in self.all_excel_data_list:
            if item.get("case_num") == self.current_case.get("case_num"):
                ...
                item["temporary_response_json"] = response.json()#接口response值存temporary_response_json
                # 如果去请求头中取数据都存一份
            ...
            for rule in rule_list:
                ...
                for line in self.all_excel_data_list:
                    if line.get("case_num") == case_num:
                        temp_data = line.get("temporary_{0}".format(params))#获取new_info
                        ...
                        #re替换模块：1：正则匹配式，2：new_info，3：old_info，4：替换匹配到的第一个
